...
module.exports = class AbruptTranslation {
    ...
    updateTranslatingStatus() {
        ...
        if (this.translating > 0) {
            console.log(`🔄 翻译中... (${this.translating}条消息)`);
        }
    }
    ...
    async translateMessage(messageElement) {
        ...
        try {
            ...
            if (textData.isComplex) {
                ...
                const loadingEl = this.insertTranslation(messageElement, "🔄 翻译中...", true);
    ...
    async translateComplexMessage(messageElement, textData) {
        ...
        loadingDiv.textContent = "🔄 正在翻译引用和回复内容...";
    ...
    createSettingsPanel() {
        ...
        panel.innerHTML = `
            <h3 style="margin: 0 0 20px 0; color: #5865f2;">🌌 AbruptTranslation 设置</h3>
            ...
            <div style="margin-bottom: 15px;">
                <label style="display: block; margin-bottom: 5px; font-weight: 600;">Base URL:</label>
            ...
            <div style="margin-bottom: 15px;">
                <label style="display: block; margin-bottom: 5px; font-weight: 600;">模型:</label>
            ...
            <div style="margin-bottom: 15px;">
                <label style="display: block; margin-bottom: 5px; font-weight: 600;">目标语言:</label>
                <select id="targetLang" style="width: 100%; padding: 8px; border: 1px solid #ddd; border-radius: 4px;">
                    <option value="zh-CN" ${this.settings.targetLang === 'zh-CN' ? 'selected' : ''}>🇨🇳 中文</option>
                    <option value="en" ${this.settings.targetLang === 'en' ? 'selected' : ''}>🇺🇸 English</option>
                    <option value="ja" ${this.settings.targetLang === 'ja' ? 'selected' : ''}>🇯🇵 日本語</option>
                    <option value="ko" ${this.settings.targetLang === 'ko' ? 'selected' : ''}>🇰🇷 한국어</option>
                </select>
            </div>
            
            <div style="margin-bottom: 20px;">
                <label style="display: block; margin-bottom: 5px; font-weight: 600;">翻译提示词:</label>
            ...
            <div style="margin-bottom: 15px; padding: 12px; background: #f0f8ff; border-radius: 6px; border-left: 4px solid #5865f2;">
                <p style="margin: 0; color: #2c5282; font-size: 14px;">
                    👆 <strong>使用说明：</strong>双击任意消息翻译，再次双击隐藏译文。<br>
                    <strong>新特性：</strong>智能分离引用和回复，视觉区分更清晰。
                </p>
            </div>
            
            <div style="display: flex; gap: 10px;">
                <button id="saveBtn" style="flex: 1; padding: 10px; background: #5865f2; color: white; border: none; border-radius: 4px; cursor: pointer; font-weight: 600;">保存设置</button>
                <button id="testBtn" style="flex: 1; padding: 10px; background: #4caf50; color: white; border: none; border-radius: 4px; cursor: pointer; font-weight: 600;">测试API</button>
                <button id="clearBtn" style="padding: 10px; background: #f44336; color: white; border: none; border-radius: 4px; cursor: pointer; font-weight: 600;">清除翻译</button>
            </div>
        ...
        panel.querySelector("#clearBtn").onclick = () => {
            this.clearAllTranslations();
            this.showStatus(panel, "已清除所有翻译", "success");
        };
    ...
    saveSettingsFromPanel(panel) {
        ...
        this.saveSettings();
        this.showStatus(panel, "设置已保存", "success");
    }
    
    async testAPIFromPanel(panel) {
        try {
            this.showStatus(panel, "测试中...", "info");
            const result = await this.callAPI("Hello");
            this.showStatus(panel, `测试成功！翻译结果: ${result}`, "success");
        } catch (error) {
            this.showStatus(panel, `测试失败: ${error.message}`, "error");
        }
    }
